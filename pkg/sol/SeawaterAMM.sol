...
contract SeawaterAMM is ISeawaterAMM {
    modifier onlyProxyAdmin() {
        require(msg.sender == StorageSlot.getAddressSlot(PROXY_ADMIN_SLOT).value, "only proxy admin");
        _;
    }
    ...
    function swapInPermit2CEAAB576(
        ...
        (bool success, bytes memory data) = _getExecutorSwapPermit2A().delegatecall(
            abi.encodeCall(
                ISeawaterExecutorSwapPermit2A.swapPermit2EE84AD91,
                (token, true, int256(amountIn), type(uint256).max, nonce, deadline, maxAmount, sig)
            )
        );
        require(success, string(data));
    ...
    function swapOut5E08A399(address token, uint256 amountIn, uint256 minOut) external returns (int256, int256) {
        (bool success, bytes memory data) = _getExecutorSwap().delegatecall(
            abi.encodeCall(ISeawaterExecutorSwap.swap904369BE, (token, false, int256(amountIn), type(uint256).max))
        );
        require(success, string(data));
    ...
    function swapOutPermit23273373B(
        ...
        (bool success, bytes memory data) = _getExecutorSwapPermit2A().delegatecall(
            abi.encodeCall(
                ISeawaterExecutorSwapPermit2A.swapPermit2EE84AD91,
                (token, false, int256(amountIn), type(uint256).max, nonce, deadline, maxAmount, sig)
            )
        );
        require(success, string(data));
